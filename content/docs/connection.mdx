---
title: Connection
description: Connect Mondoo to MongoDB via Cloudflare Durable Objects
---

# Connection

Mondoo is designed for Cloudflare Workers with Durable Objects. It provides a connection adapter that works with `mondodb` (MongoDB-compatible API for Durable Objects).
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    // Create mondoo instance with Worker environment
...
```typescript
import { model, Schema } from 'mondoo'
...
const connection = mondoo.connection
```

### Connection Events

```typescript
mondoo.connection.on('connected', () => {
  console.log('Connected to database')
})

mondoo.connection.on('error', (err) => {
  console.error('Connection error:', err)
})

mondoo.connection.on('disconnected', () => {
...
const userDb = createMondoo(env, { database: 'users' })
const analyticsDb = createMondoo(env, { database: 'analytics' })
...
```typescript
import { model, getModel, hasModel, deleteModel, modelNames } from 'mondoo'
...
Mondoo supports transactions via sessions:

```typescript
const session = await mondoo.startSession()
...
```typescript
await mondoo.connection.transaction(async (session) => {
  const user = await User.create([{ name: 'John' }], { session })
  const post = await Post.create([{
    title: 'Hello',
    author: user[0]._id
...
const isConnected = mondoo.connection.readyState === 1

// Ping the database
try {
  await mondoo.connection.db.admin().ping()
...
export default {
  async fetch(request: Request, env: Env) {
    // Create fresh mondoo per request
    const mondoo = createMondoo(env)
    const User = mondoo.model('User', userSchema)
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    // Models are cached per mondoo instance
    const User = mondoo.model('User', userSchema)
    const Post = mondoo.model('Post', postSchema)
...
```typescript
// types.ts
import type { Schema } from 'mondoo'
...
export default {
  async fetch(request: Request, env: Env) {
    // Only create mondoo and register models inside handler
    const mondoo = createMondoo(env)
    const User = mondoo.model('User', userSchema)
